...
/**
 * Bluetooth-optimised transport using Nearby Connections P2P_STAR strategy.
 *
 * - Lower power, shorter range.
 * - Only connects to nodes whose ID starts with "BT-".
 * - Chunk size: 512 bytes (safe BLE ATT MTU limit).
 */
@Singleton
class BluetoothNearbyTransport @Inject constructor(
    ...
    companion object {
        const val SERVICE_ID = "com.meshchat.bluetooth"
        const val NODE_PREFIX = "BT-"
    }
    ...
    override fun start(localNodeId: String) {
        require(localNodeId.startsWith(NODE_PREFIX)) {
            "BluetoothNearbyTransport requires a BT-prefixed nodeId, got: $localNodeId"
        }
    ...
    override fun stop() {
        ...
        Timber.d("BluetoothTransport stopped")
    }
    ...
    private fun startAdvertising() {
        ...
        connectionsClient.startAdvertising(
            ...
            isAdvertising = true
            Timber.d("BT advertising started — ...${nodeId.takeLast(4)}")
        }.addOnFailureListener { e ->
            if (e is ApiException && e.statusCode == ConnectionsStatusCodes.STATUS_ALREADY_ADVERTISING) {
                isAdvertising = true
            } else {
                Timber.e(e, "BT advertising failed — retrying")
                retryAdvertisingJob?.cancel()
    ...
    private fun startDiscovery() {
        connectionsClient.startDiscovery(
            ...
            if (e is ApiException && e.statusCode == ConnectionsStatusCodes.STATUS_ALREADY_DISCOVERING) {
                ...
                Timber.e(e, "BT discovery failed — retrying")
                retryDiscoveryJob?.cancel()
    ...
    private val endpointDiscoveryCallback = object : EndpointDiscoveryCallback() {
        override fun onEndpointFound(endpointId: String, info: DiscoveredEndpointInfo) {
            // Only connect to BT-prefixed nodes
            ...
            // Deterministic tie-break: lower ID initiates
            if (localId < info.endpointName) {
                connectionsClient.requestConnection(localId, endpointId, connectionLifecycleCallback)
                    .addOnFailureListener { e -> Timber.e(e, "BT connection request failed") }
            }
        }

        override fun onEndpointLost(endpointId: String) {
            Timber.d("BT endpoint lost: $endpointId")
        }
    }
    ...
    private val connectionLifecycleCallback = object : ConnectionLifecycleCallback() {
        ...
        override fun onConnectionResult(endpointId: String, resolution: ConnectionResolution) {
            ...
            if (resolution.status.isSuccess) {
                ...
                Timber.d("BT connected — ...${nodeId.takeLast(4)}")
            } else {
            ...
            scope.launch {
                runCatching {
                    ...
                    _incomingChannel.trySend(packet).also { result ->
                        if (result.isFailure) Timber.w("BT packet dropped — channel full")
                    }
                }.onFailure { Timber.e(it, "BT packet parse error") }
            }
        }
    ...
    suspend fun send(peerId: String, data: ByteArray) {
        val endpointId = nodeToEndpointMap[peerId] ?: run {
            Timber.w("BT send: no endpoint for $peerId")
            return
        }
        runCatching {
            withTimeout(3_000L) {
        ...
        }.onFailure { Timber.w(it, "BT unicast failed to ...${peerId.takeLast(4)}") }
    }
            ...
            endpoints.map { (nodeId, endpointId) ->
                async {
                    runCatching {
                        ...
                        Timber.w(it, "BT broadcast failed — removing ...${nodeId.takeLast(4)}")
    ...
    private suspend fun sendPresencePacket(peerNodeId: String) {
        ...
        }.onFailure { Timber.e(it, "BT presence send failed") }
    }
}
